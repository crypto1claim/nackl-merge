...
const TRANSLATIONS = {
  ru: {
    ...
    'about.earn_text': 'Финальная монета в лестнице — NACKL, фирменный токен сети Acki Nacki. Когда ты её собираешь, ты делаешь свой главный вклад в безопасность сети. Будущая версия игры позволит конвертировать MRG в настоящие токены $NACKL.',
    ...
    'about.bomb_text': 'Спецспособность для критических моментов. Шкала Shake Damage копится из каскадов слияний (комбо ×3 и выше). Чем больше каскад, тем больше прибавка. Когда шкала заполнится до 100% — превращается в готовый заряд (точка загорается). Заряд тратится по нажатию кнопки с молнией или по тряске телефона: все монеты в банке взрываются волной из центра, ты получаешь крупный бонус MRG. Лимит партии — 3 использования. После 3-х новые заряды больше не накапливаются.',
    'about.support_title': 'Поддержать автора',
    'about.support_text': 'Игра сделана независимым разработчиком в экосистеме Acki Nacki. Если она тебе нравится — подпишись на канал и поддержи разработку.',
  ...
  en: {
    ...
    'about.earn_text': 'The final coin in the ladder is NACKL — the flagship token of the Acki Nacki network. When you reach it, you make your biggest contribution to network security. A future version will let you convert MRG into real $NACKL tokens.',
    ...
    'about.bomb_title': '💥 Shake Damage',
    'about.bomb_text': 'A special ability for critical moments. The Shake Damage gauge fills up from big merge cascades (combos ×3 and higher). When the gauge fills to 100%, it converts into one ready charge (a dot lights up). Spend a charge by tapping the lightning button or shaking your phone: all coins in the jar explode in a wave from the center, giving you a big MRG bonus. Limit per game — 3 uses. After 3 uses, no more charges accumulate and the game continues without Shake Damage.',
    'about.support_title': 'Support the author',
    'about.support_text': 'The game is made by an independent developer in the Acki Nacki ecosystem. If you like it — subscribe to the channel and support development.',
